...
Vector3f Scene::castRay(const Ray& ray_in, int depth) const {
	...
	Intersection intersection = Scene::intersect(ray_in);
	...
	if (intersection.happened) {
		...
		hitObject->getSurfaceProperties(hitPoint, ray_in.direction, index, uv, N, st);
		//        Vector3f tmp = hitPoint;
		switch (m->getType()) {
		case REFLECTION_AND_REFRACTION: {
			Vector3f reflectionDirection = normalize(reflect(ray_in.direction, N));
			Vector3f refractionDirection = normalize(refract(ray_in.direction, N, m->ior));
			...
			float kr;
			fresnel(ray_in.direction, N, m->ior, kr);
		...
		case REFLECTION: {
			float kr;
			fresnel(ray_in.direction, N, m->ior, kr);
			Vector3f reflectionDirection = reflect(ray_in.direction, N);
			Vector3f reflectionRayOrig = (dotProduct(reflectionDirection, N) < 0)
				                             ? hitPoint + N * EPSILON
				                             : hitPoint - N * EPSILON;
		...
		default: {
			...
			Vector3f shadowPointOrig = (dotProduct(ray_in.direction, N) < 0)
				                           ? hitPoint + N * EPSILON
				                           : hitPoint - N * EPSILON;
			// [comment]
			...
			for (uint32_t i = 0; i < get_lights().size(); ++i) {
				...
				if (area_ptr) {
					...
					specularColor += powf(std::max(0.f, -dotProduct(reflectionDirection, ray_in.direction)),
